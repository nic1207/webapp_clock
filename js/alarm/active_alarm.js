 ...
 */
  ...
  init: function am_init() {
    ...
    if (jQuery.isFunction(navigator.mozSetMessageHandler))
    {
  ...
  onAlarmFiredHandler: function aac_onAlarmFiredHandler(message) {
    ...
    AlarmManager.getAlarmById(id, function aac_gotAlarm(alarm) {
      ...
      // prepare to pop out attention screen, ring the ringtone
  ...
  onWebAlarmFiredHandler: function(alarm) {
    this._onFireAlarm = alarm;
    RingWebView.init(alarm);
    RingWebView.show();
  },
  
  WebsnoozeHandler: function() {
    var id = this._onFireAlarm.id;
    AlarmManager.getAlarmById(id, function(alarm) {
      alarm.enabled = true;
      AlarmManager.putAlarm(alarm, function(alarm) {
